...
namespace Sample
{
  ...
  template <typename T>
  TemplatedSequenceContext<T>::TemplatedSequenceContext() : m_rank(-1),
  ...
  template <typename T>
  bool TemplatedSequenceContext<T>::iterateUntilRank(int n, SampleStore *sequenceStore, SequenceContext *sqctx)
  {
    if (m_rank > n)
    {
      m_rank = -1;
  ...
  template <typename T>
  void TemplatedSequenceContext<T>::step(SampleStore *sequenceStore, SequenceContext *sqctx)
  {
    /* Shift values */
    ...
    for (int i = 0; i < sequenceStore->numberOfModels(); i++)
    {
      ...
      sequences[SampleStore::sequenceIndexForName(u->fullName()[0])] = u->isDefined() ? u : nullptr;
    }
