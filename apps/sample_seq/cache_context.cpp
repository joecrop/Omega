...
namespace Sample
{
  ...
  template <typename T>
  CacheContext<T>::CacheContext(Context *parentContext) : ContextWithParent(parentContext),
  ...
  {
    // [u|v|w](n(+1)?)
    if (symbol.type() == ExpressionNode::Type::Symbol && symbol.name()[0] >= SampleStore::k_sequenceNames[0][0] && symbol.name()[0] <= SampleStore::k_sequenceNames[MaxNumberOfSequences - 1][0] && (strcmp(symbol.name() + 1, "(n)") == 0 || strcmp(symbol.name() + 1, "(n+1)") == 0))
    {
  ...
  int CacheContext<T>::nameIndexForSymbol(const Poincare::Symbol &symbol)
  {
    ...
    assert(name >= SampleStore::k_sequenceNames[0][0] && name <= SampleStore::k_sequenceNames[MaxNumberOfSequences - 1][0]); // u, v or w
    return name - 'u';
  }
