...
namespace Sample
{
  ...
  SequenceToolbox::SequenceToolbox() : MathToolbox(),
                                       m_addedCellLayout{},
   ...
   * want to parse symbols u(n+2), v(n+2) or w(n+2). */
    m_numberOfAddedCells = 0;
    int sequenceIndex = SampleStore::sequenceIndexForName(sequenceName[0]);
    for (int i = 0; i < MaxNumberOfSequences; i++)
    {
      for (int j = 0; j < recurrenceDepth + 1; j++)
      {
        ...
        m_addedCellLayout[m_numberOfAddedCells++] = HorizontalLayout::Builder(
            CodePointLayout::Builder(SampleStore::k_sequenceNames[i][0], KDFont::LargeFont),
